...
export const DialogProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <DialogContext.Provider value={{ isOpened: showDialog, openDialog, setIsOpened: setShowDialog, title, confirmDialog: showSuccessDialog }}>
      {children}
      {showDialog && (
        <Dialog title={title} isOpen={showDialog} onClose={() => setShowDialog(false)} width="444px">
          <div className="flex flex-col gap-4">
            {subtitle !== '' && <div className="text-base-8 text-xl font-semibold ">{subtitle}</div>}
            {description !== '' && <div className="text-base-4 text-base font-medium ">{description}</div>}
