...
export const ocurrrenceClassificationService = {
  async getClassifications(): Promise<IOcurrenceClassification[]> {
    const response = await api.get<IOcurrenceClassification[]>(`/occurrence-classifications`);

    return response.data;
  },

  async getClassificationsByType(typeId: string | number): Promise<IOcurrenceClassification[]> {
    const response = await api.get<IOcurrenceClassification[]>(`/occurrence-classifications/${typeId}`);
  ...
  async insertClassification(classification: IOcurrenceClassification): Promise<void> {
    try {
  ...
  async updateClassification(classification: IOcurrenceClassification): Promise<void> {
    try {
