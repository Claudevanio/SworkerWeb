...
export const ocurrenceTypeService = {
  ...
  async getTypesWithPagination(term: string, currentPage: number, pageSize: number): Promise<basePagination<IOcurrenceType>> {
    const response = await api.get<any>(`/occurrence-type?pageSize=100`, {
      params: {
        term
      }
  ...
  async updateType(type: IOcurrenceType): Promise<void> {
    try {
