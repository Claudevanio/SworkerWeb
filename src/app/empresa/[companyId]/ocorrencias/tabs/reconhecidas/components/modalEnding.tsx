...
export default function ModalEnding({
  ...
  const methodsRecognize = useForm<FormFields>({
    ...
    defaultValues: {
      ...
      status: currentOcurrence.occurrence?.auditStatus ? 'Concluido' : 'Pendente'
    }
  });

  return (
    <Form onSubmit={onSubmit} className="flex flex-col gap-4 pb-4" {...methodsRecognize}>
      ...
      <div className="flex gap-4 md:gap-6 flex-col md:flex-row justify-between">
        ...
        <Input name="dateAndTime" label="Data e Hora" required disabled={true} />
        <Input name="professional" label="Profissional" required disabled={true} />
      </div>

      <div className="flex gap-4 md:gap-6 flex-col md:flex-row justify-between">
        <Input name="characterization" label="Caracterização" required disabled={true} />
