...
export function Encerradas({
  ...
  const { isLoading, data: ocurrences } = useQuery<basePagination<IOcurrenceRecognize> | undefined>({
    queryKey: ['encerradas', { filterOcurrences, filter }],
    queryFn: () =>
      ocurrenceService.listOcurrenceRecognitionAsync(currentCompany?.id, filter.term, filter.page, filter.pageSize, filterOcurrences, true) as any,
  ...
  return (
    <Stack>
      <Stack flexDirection={isMobile ? 'column' : 'row'} justifyContent="space-between">
        ...
        {!isMobile && (
          <ExportButton disabled={selected.length < 1} fileName="ocorrencias-encerradas.csv" csvData={exportOcurrences} hidden={isMobile} />
        )}
      </Stack>
