...
export default function ModalEdit({
  ...
  return (
    <Form onSubmit={data => onSubmitEdit(data as FormFieldsEdit)} className="flex flex-col gap-4 pb-4" {...methodsEdit}>
      <div className="flex gap-4 md:gap-6 flex-col md:flex-row justify-between">
        <Input name="serviceOrder" label="Ordem de serviço" required disabled={true} />
        <Dropdown
      ...
      <Stack width={isMobile ? '100%' : '50%'}>
        <Input name="locale" label="Local" required placeholder="Local" error={methodsEdit.formState.errors.locale} disabled={false} />
      </Stack>
      <Input
        name="description"
        label="Descrição"
