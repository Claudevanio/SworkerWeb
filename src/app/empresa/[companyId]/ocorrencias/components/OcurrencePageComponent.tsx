...
export function OcorrenciasPageComponent() {
  const isMobile = useMediaQuery('(max-width:768px)');

  const searchParams = useSearchParams();

  const [activeTab, setActiveTab] = useState<number | undefined>(searchParams.get('tab') ? parseInt(searchParams.get('tab') as string) : 0);
  ...
  return (
    <div className="p-4 lg:p-8">
      <PageTitle title="Ocorrências" subtitle="Veja aqui todas as permissões, empresas e equipamentos cadastrados" />
      <SimpleTab tabs={tabs} activeTab={activeTab} setActiveTab={setActiveTab} mobileView />
      {activeTab == 0 && <Geradas isMobile={isMobile} classifications={classifications} characterizations={characterizations} types={types} />}
      {activeTab == 1 && <Reconhecidas isMobile={isMobile} classifications={classifications} characterizations={characterizations} types={types} />}
      {activeTab == 2 && <Encerradas isMobile={isMobile} classifications={classifications} characterizations={characterizations} types={types} />}
    </div>
  );
}
