...
export function Tipo({ isMobile, openModalAdd, handleCloseModalAdd }: { isMobile: boolean; openModalAdd: boolean; handleCloseModalAdd: () => void }) {
  ...
  const [typeSelected, setTypeSelected] = useState<IOcurrenceType>({} as IOcurrenceType);

  const {
    isLoading,
    data: types,
    refetch
  } = useQuery<basePagination<IOcurrenceType> | undefined>({
    queryKey: ['conf-tipo', { filter }],
    queryFn: () => ocurrenceTypeService.getTypesWithPagination(filter.term, filter.page, filter.pageSize) as any,
              ...
              <CheckBox
                ...
                onChange={() => {
                  if (selected.includes(type.id)) {
                    setSelected(prev => prev.filter(item => item !== type.id));
                  }
                  if (!selected.includes(type.id)) {
  ...
  const handleChangeExportTypes = () => {
    ...
    const csvData = [
      ...
      ...(arrayTpes?.map(type => {
        return [type.typeName ?? '', type.description ?? '', dayjs(type.registerDate).format('DD/MM/YYYY') ?? ''];
      }) ?? [])
    ];
  ...
  return (
    <Stack>
      <Stack flexDirection={isMobile ? 'column' : 'row'} justifyContent="space-between">
        ...
        {!isMobile && <ExportButton disabled={selected.length < 1} fileName="tipos.csv" csvData={exportTypes} hidden={isMobile} />}
      </Stack>
