...
export default function Classification({ classificationCards, isMobile }: { classificationCards: IClassificationCard[]; isMobile: boolean }) {
  ...
  return (
    <>
      <Stack flexDirection={isMobile ? 'column' : 'row'} mt={2} justifyContent={isMobile ? '' : 'space-between'}>
      ...
      <Stack mt={2} flexDirection={isMobile ? 'column-reverse' : 'row'} justifyContent="space-between">
        ...
        <Stack width="49%" gap={2} justifyContent="space-between">
          {classificationCards.slice(4, 7).map((item, index) => (
            <CardCategory hide={item.number < 0} key={index} Icon={item.icon} number={item.number} description={item.description} width={'100%'} />
          ))}
        </Stack>
      </Stack>
    </>
