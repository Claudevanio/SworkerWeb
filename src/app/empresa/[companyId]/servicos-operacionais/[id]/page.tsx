...
function generateLabelsAndValues(tasksSteps: Task[]): { label: string; value: number }[] | null {
  ...
  while (currentDate.isBefore(lastDay)) {
    const label = currentDate.format(
      interval === 'day' ? 'DD/MM/YYYY' : interval === 'hour' ? 'DD/MM/YYYY HH:mm' : 'HH:mm:ss'
    );
...
export default function ServicoDetailPage(){
  ...
  const fetchProfessionalData = useCallback(async (professionalId: number) => {
    try {
  ...
  const tasksFinishedData = (tasksSteps ?? []).map(
    item => {
      return {
        label: item.code,
  ...
  const tasksNotFinishedData = (tasksSteps ?? []).map(
    item => {
      return {
        label: item.code,
        value: item.steps.filter(step => !step.executed).length,
  ...
  return (
    <div
      className='w-full p-4 lg:p-8 flex flex-col gap-8'
    >
      <Link
        href='/servicos-operacionais'
      >
        <KeyboardArrowLeft
          className='text-base-8 cursor-pointer'
          fontSize='large'
        />
      </Link> 
      <DetailCard.Card>
        ...
        >
          ...
          <DetailCard.Field
            ...
            value={serviceDetail?.checkInDate ?
              dayjs(serviceDetail.checkInDate).format('DD/MM/YYYY : HH:mm') : ''}
          />

          <DetailCard.Field
            label='Check-in'
            value={serviceDetail?.checkInEndereco ?? ''}
          />

          <DetailCard.Field
            label='Check-out'
            className=''
            value={data?.checkOutEndereco ?? ''}
          />
          <DetailCard.Field
            label=''
            value={''}
          ...
          <DetailCard.Field
            label='Equipe'
            value={
              serviceDetail?.professionals.map((professional) => professional.name)
            }
          />

          <DetailCard.Field
            label='Responsável'
            value={serviceDetail?.supervisor ?? ''}
          />

          <DetailCard.Field
            label='Data de encerramento'
            value={data?.checkOutDate ?
              dayjs(data.checkOutDate).format('DD/MM/YYYY : HH:mm') : ''}
          />

          <DetailCard.Field
            label='Recursos'
            value={serviceDetail?.equipments.map((equipment) => equipment.model)}
          />
                ...
                ocurrencesTableData.map((ocurrence : IOcurrence) => {
                  return (
                    ...
                    >
                      ...
                      <div
                        className='px-4 py-4 md:py-6 md:px-12 grid grid-cols-2'>
                          <DetailCard.Field
                            label='Número'
                            value={ocurrence.number?.toString() ?? ''}
                          />
                          <DetailCard.Field
                            label='Observações'
                            value={
                              (!ocurrence.observation || ocurrence.observation === '') ? 
                              'Sem observações' : ocurrence.observation
                            }
                          />
                          <DetailCard.Field
                            label='Procedimento/Serviço'
                            value={ocurrence.serviceOrder?.description ?? ''}
                          />
                          <DetailCard.Field
                            label='Data'
                            value={ocurrence.registerDate ?
                              dayjs(ocurrence.registerDate).format('DD/MM/YYYY : HH:mm') : ''}
                          />
                          <DetailCard.Field
                            label='Local'
                            value={ocurrence.local ?? ''}
                          />
                          <DetailCard.Field
                            label='Tipo'
                            value={ocurrence.occurrenceType.typeName ?? ''}
                          />
            ...
            serviceDetail?.professionals?.map((professional) => {
              return (
                ...
                >
                  ...
                  /> 
                  <div
                    className='flex flex-row gap-4 overflow-auto'
                  >
                    <div
                      className='w-full min-w-[1200px] mx-2 mr-4'
                    >
                      <LineChart
                  ...
                  >
                    {
                      tasksSteps.map((task, index) => {
                        ...
                        >        
                            <div
                              className='w-full p-4 bg-primary-200'
                            >
                            ...
                            </div>
                            {
                              task.steps.map((step, stepIndex) => {
                                return (
                                  ...
                                  >
                                    <div
                                      className='!w-8 !h-8 shrink-0 bg-primary-500 rounded-full flex justify-center items-center text-white'
                                    >
                                      {step.number}
                                    </div>
                                    <div
                                      className='flex flex-col gap-2'>
                                        <p
                                          className='text-base-5 font-semibold'
                                        >
                                          {step.description}
                                        </p>
                                        <p
      ...
      <ModalOcurrenceDetail
        isOpen={isModalOpen}
        onClose={closeModal}
        current={selectedOcurrence}
        readonly
      />
    </div>
  );
