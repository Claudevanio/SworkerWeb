...
export function ModalOcurrenceDetail({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={current?.description ?? 'Ocorrência'} width="1000px">
      <Form onSubmit={data => console.log(data)} className="flex flex-col gap-4 pb-4" {...methods}>
        <div className="flex flex-col md:flex-row gap-4 w-full md:justify-between">
          <Input label="Número" name="number" disabled={readonly} />

          <Input label="Data" name="registerDate" disabled={readonly} />

          <Input label="Local" name="local" disabled={readonly} />

          <Input label="Tipo" name="occurrenceType.typeName" disabled={readonly} />
        </div>
        <Input label="Descrição" name="description" disabled={readonly} minRows={4} multiline />

        <Input label="Observação" name="observation" disabled={readonly} minRows={4} multiline />
      </Form>
    </Modal>
  );
}
