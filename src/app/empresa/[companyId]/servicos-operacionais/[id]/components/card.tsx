...
function Title({ children, wrap }: { children: React.ReactNode | React.ReactNode[]; wrap?: boolean }) {
  return (
    <Box
      className="flex flex-row items-center w-full bg-primary-100 rounded-t-lg py-6 px-8 "
      sx={{
...
function Card({ children }: { children: React.ReactNode | React.ReactNode[] }) {
  return <div className="border-2 border-primary-200 rounded-lg">{children}</div>;
}
    ...
    <div className={`flex flex-col gap-2 ${className}`}>
      ...
      {Array.isArray(value) && (
        <div className="flex flex-col gap-2">
          {value.map((v, index) => (
            <span key={index} className="text-base-7">
              {v}
            </span>
          ))}
        </div>
