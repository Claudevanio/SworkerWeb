...
export function ProfilePageComponent() {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (data.newPassword && data.confirmPassword) {
      try {
        await Userservice.updateLoggedUserPassword(user.userId, {
          userName: user.email,
  ...
  return (
    <div className="w-full p-4 lg:p-8 pb-12">
      <PageTitle title="Meus Dados" subtitle="Edite alguns de seus dados cadastrais" />
      <div>
        <Form
          {...methods}
        ...
        >
          <div className="flex flex-col md:flex-row gap-4 md:gap-8  justify-between">
            <Input label="Nome" name="name" required error={methods.formState.errors.name} />
          ...
          <div className="flex flex-col md:flex-row gap-4 md:gap-8  justify-between">
            <Input label="Telefone" mask={masks.TELEFONEMask} name="phone" required error={methods.formState.errors.phone} />

            <Input label="Perfil" name="profile" required disabled />

            <Input label="Unidade" name="unity" required disabled />
          </div>
          <div className="flex flex-col md:flex-row gap-4 md:gap-8  justify-between">
            <Input label="Senha Atual" name="password" type="password" required error={methods.formState.errors.password} />
            <Input label="Nova Senha" name="newPassword" type="password" required error={methods.formState.errors.newPassword} />
            <Input label="Confirmar Senha" name="confirmPassword" type="password" required error={methods.formState.errors.confirmPassword} />
          </div>
          <div className="flex justify-end">
            <Button type="submit" isLoading={methods.formState.isSubmitting} disabled={!isFormChanged} className="w-full md:w-auto">
              Salvar
            </Button>
          </div>
        </Form>
