...
export function ResourcesStep({
  ...
  return (
    ...
    <div
    className='flex flex-col border-[1px] rounded-lg p-1 border-base-4 md:border-none md:flex-row gap-4 md:gap-6'
  >
    <AutoComplete  
      label='Recursos'
      ...
      options={
        (tasksResources.data ?? []).map((t: IEquipmentClassification) => ({name: t.description, value: t.id}))
      }
      onSelect={(value: {
        name: string;
        value: string; 
    ...
    <Input
      ...
      label='Classificação'
      value={
        currentEquipment.classification
      }
    />
