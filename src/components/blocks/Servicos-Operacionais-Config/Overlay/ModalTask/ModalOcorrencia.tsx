...
export function ModalOcorrencia({
  ...
  const {
    ...
    queryFn: () => ocurrenceCharacterizationService.getCharacterizationsByTypeAsync(
      typeId
    ),
    refetchOnWindowFocus: false,
  ...
  const {
    data: ocurrences,
    isLoading: isLoadingOcurrences,
  } = useQuery({
    queryKey: ['ocurrences', characterizationId],
    queryFn: () => ocurrenceService.getOcurrenceByCategoryAsync(
      characterizations.find((t: any) => t.id == characterizationId)?.description
    ),
    refetchOnWindowFocus: false,
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={
        current ? readonly ? current?.id + '' : 'Editar Ocorrencia' : 'Escolher Ocorrencia'
      }
      width='550px'
    ...
    >
      ...
      >
        ...
        <Dropdown
            ...
            options={ocurrences ? 
              ocurrences.map((t: any) => ({ label: t.description, value: t.id })) 
              : []}
            required
          />
