...
export function ModalTask({ isOpen, onClose, current, readonly }: { isOpen: boolean; onClose: () => void; current?: ITask; readonly?: boolean }) {
  ...
  async function onSubmit(data: any) {
    ...
    const resourcesToSend = resourcesList
      .filter(resource => {
    ...
    try {
      ...
      await Promise.all(
        resourcesToSend.map(async resource => {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      methods={methods}
      title={current ? (readonly ? current.name : 'Editar Tarefa') : 'Nova Tarefa'}
      width="1000px"
    ...
    >
      ...
      <Form onSubmit={data => onSubmit(data as FormFields)} className="flex flex-col gap-4 pb-4" {...methods}>
        {currentStep === 1 && (
          <div className="flex flex-col gap-6">
            ...
            {currentOcurrence && generateOcurrence && (
              <CustomizedGroupAccordion filledSummary summary="Ocorrência">
                <div className="p-6 flex flex-col gap-4">
                  <Input disabled={true} label={'ID ' + currentOcurrence?.id} defaultValue={currentOcurrence?.description} />
                  <p className="text-primary-700 underline cursor-pointer" onClick={openModalOcorrencia}>
                    Editar
                  </p>
                </div>
        ...
        {currentStep === 2 && (
          <div className="flex flex-col gap-4">
            <div className="flex justify-end w-full gap-4">
              <Button onClick={openNewStepModal}>
                <Add />
                Novo Passo
              </Button>
            </div>
      ...
      <ModalOcorrencia
        isOpen={isModalOcorrenciaOpen || (generateOcurrence && generateOcurrence !== '' && !currentOcurrence)}
        onClose={() => {
          closeModalOcorrencia();
          if (!currentOcurrence) methods.resetField('isOccurrenceIdIncomplete');
        }}
        onSubmit={(data: IOcurrence) => {
