...
export function ModalTaskView({
  ...
  return (
    <Modal
      ...
      methods={methods}
      title={
        current?.name ?? 'Visualizar Tarefa'
      }
      width='1000px'  
    > 
 ...
 <div
        ...
        >
          ...
          {(fetchOcurrence.data as any)?.length > 0 && <CustomizedGroupAccordion
          filledSummary
          summary='Ocorrência'	
          > 
          <div
          ...
          >
            <Input
              disabled={true}
              label={'ID ' + fetchOcurrence.data?.[0]?.id}
              defaultValue={
                fetchOcurrence.data?.[0]?.description
              } 
            /> 
          </div>
          ...
          className='flex flex-col gap-4'> 
            {
              resourcesList && resourcesList.map((resource, index) => (
                <div
                  key={index}
                  className='flex gap-4 items-center justify-between bg-base-2 border-base-3 border-[1px] px-4 py-2 rounded-lg'
                > 
                  <p>
