...
const FieldAccordion = ({ label, name }: { label: string; name: keyof FormFields }) => (
  <Accordion defaultExpanded={false} className="shadow-none gap-0">
    <AccordionSummary
    ...
    <AccordionDetails className="px-0">
      <Input label={''} name={name} multiline minRows={3} />
    </AccordionDetails>
  </Accordion>
);
...
export function ModalNewStep({
  ...
  async function onSubmitForm(data: any) {
    ...
    if (current?.draggable) data.draggable = current.draggable;
    onSubmit(data);
  }
  ...
  const { data: serchedTasks } = useQuery({
    queryKey: ['tasks', term],
    queryFn: () =>
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={current ? (readonly ? current?.id + '' : 'Editar Passo') : 'Novo Passo'}
      width="900px"
    ...
    >
      <Form onSubmit={data => onSubmitForm(data as FormFields)} className="flex flex-col gap-4 pb-4" {...methods}>
        <div className="flex flex-col md:flex-row gap-4 justify-between w-full">
          <Input label="Número" mask={masks.NUMBER} name="number" required />
          <Dropdown
            label="Tipo"
            name="taskTypeId"
            options={(taskTypes.data ?? []).map((t: ITaskType) => ({ label: t.description, value: t.id }))}
            required
          />
          <Dropdown
            label="Subtipo"
        ...
        <div className="flex flex-col gap-4">
          {textAreaFields.map(field => (
            <div key={field.name} className="w-full">
              <FieldAccordion label={field.label} name={field.name as keyof FormFields} />
            </div>
          ))}
        </div>
        ...
        {isQuestion && (
          <div className="flex flex-col gap-4">
            {alternatives?.map((alternative: any, index: number) => (
              <CustomizedGroupAccordion summary="Alternativa" key={index} filledSummary>
                <div className="flex gap-4">
                  <Controller
                    control={methods.control}
                    name={`alternatives.${index}.hasImage`}
                    render={({ field }) => (
                      <CheckBox label="Exige foto" variant="secondary" value={field.value} onChange={() => field.onChange(!field.value)} />
                    )}
                  />
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <CheckBox label="Finaliza APR" variant="secondary" value={field.value} onChange={() => field.onChange(!field.value)} />
                    )}
                  />
                ...
                <div className="flex flex-col gap-4">
                  {alternative.alternative.description && alternative.alternative.description !== '' && (
                    <AutoComplete
                      ...
                      style={{ width: '100%' }}
                      options={serchedTasks?.items.map((t: any) => ({ name: t.name, value: t.id })) ?? []}
                    />
                  )}
                </div>
