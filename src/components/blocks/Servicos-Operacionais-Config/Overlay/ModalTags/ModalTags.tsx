...
)
{
  ...
  const handleSave = async () => { 
    ...
    if(hasChanged){
      await Promise.all(tagTasks.map(async (task, index) => {
  ...
  const fetchTagsTasks = async () => {
    if(!methods.getValues('uid') || methods.getValues('uid') === '')
      return;

    const response = await TagsService.getTagsSteps(methods.getValues('uid'));
    const filteredResponse = response.filter((step: ITagsTasks) => step.tagId === currentId).map((step: ITagsTasks) => {
      return {
        ...step,
        id: Math.random(),
        taskId: step.task.id,
  ...
  return (
    <Modal
      ...
      methods={methods}
      title={
        current ? readonly ? current.description : 'Editar Procedimento' : 'Novo Procedimento'
      }
      width='1000px'
