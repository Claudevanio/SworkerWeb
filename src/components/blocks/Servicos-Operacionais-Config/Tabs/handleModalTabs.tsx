...
export function ModalTabs({ tab }: { tab: number }) {
  const { modal, contexts, taskGroups, tasks, tags } = useServiceOperations();
  ...
  if (tab === 2)
    return (
      <ServicosOperacionaisConfigModals.ModalTask isOpen={modal.isOpen} onClose={modal.close} current={tasks.current} readonly={tasks.readonly} />
    );
  if (tab === 3)
    return <ServicosOperacionaisConfigModals.ModalTags isOpen={modal.isOpen} onClose={modal.close} current={tags.current} readonly={tags.readonly} />;
}
