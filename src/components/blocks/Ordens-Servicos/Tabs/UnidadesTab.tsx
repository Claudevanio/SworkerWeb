...
export const UnidadesTab = ({ openFilterModal, serviceOrders }: { openFilterModal: () => void; serviceOrders: IServiceOrderDay[] }) => {
  ...
  const units =
    serviceOrders
      ?.reduce((uniqueUnits, item) => {
        const unitName = item?.unityDescription;
        const unitId = item?.unityId;

        if (!uniqueUnits.some(professional => professional.id === unitId)) {
          uniqueUnits.push({ name: unitName, id: unitId });
        }
  ...
  const paginatedUnits = usePagination(units ?? [], 5);
  ...
  const filterServiceOrders = useCallback((serviceOrders: IServiceOrderDay[], criteria: FilterCriteria): IServiceOrderDay[] => {
    ...
    return serviceOrders.filter(order => {
      if (criteria.unit && order.unityId !== +criteria.unit) {
        return false;
      }
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <CustomizedGroupAccordion summary="Unidades">
        <div className="p-4 flex flex-col gap-8">
          <div className="grid md:grid-cols-3 lg:grid-cols-5 gap-4">
            {paginatedUnits.currentTableData?.map(item => {
              return (
                <div
                  key={item.id}
          ...
          <Pagination
            ...
            onChange={page => paginatedUnits.setCurrentPage(page)}
            totalPages={paginatedUnits.totalPage}
          />
        </div>
      ...
      {selectedGroup ? (
        ...
        <div className="flex items-center justify-center flex-col gap-2 p-4">
          <Image src="/Warning.svg" width={40} height={40} alt="warning" />
          <p className="text-base-4 font-medium">Selecione uma unidade para visualizar as ordens de serviço</p>
        </div>
      )}
    </div>
  );
