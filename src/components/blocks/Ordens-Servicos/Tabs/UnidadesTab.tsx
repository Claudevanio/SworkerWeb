...
import { basePagination, ICompanyUnity, IProfessional } from '@/types';
import { companyUnityService, professionalService } from '@/services';
...
export const UnidadesTab = (
  ...
  const units = useQuery({
    queryKey: ['UnityCompany'],
    queryFn: () => companyUnityService.getAll(),
  ...
  const unitsPaginated  = usePagination(units.data ?? [], 5);

  const [selectedGroup, setSelectedGroup] = useState<ICompanyUnity | null>(null); 

  useEffect(() => {
    if (units.data && !selectedGroup) {
      if(units.data.length > 0)
        setSelectedGroup(units.data[0]);
    }
  }, [
    units.data,
  ])
  ...
  const {
    ...
    queryKey: ['AgrupamentoByProfessional', currentProfessional?.id],
    queryFn: () => serviceOrderService.getServiceOrderByProfessionalIdAsync(currentProfessional?.id ?? ''),
  ...
  const professionalsBySelectedGroup = professionals.data?.filter((professional : any) => professional.companyUnityId === selectedGroup?.id);
        ...
        >  
          ...
          >
            ...
            >
              ...
              >
                {
                  units.isLoading && new Array(5).fill(0).map((_, index) => { 
                    return (
                      <div
                        key={index}
                ...
                {
                  unitsPaginated.currentTableData?.map((item) => {
                    return (
                      <div
                        key={item.id}
                        className={`flex gap-2 items-center p-6 cursor-pointer rounded-lg transition-all ${
              ...
              <Pagination
                currentPage={unitsPaginated.currentPage}
                onChange={(page) => unitsPaginated.setCurrentPage(page)}
                totalPages={unitsPaginated.totalPage}
              />
            </div>
          </CustomizedGroupAccordion>

          <p
            className='text-base-4 font-medium '
          >
            {
              selectedGroup?.name
            }
          ...
          {
            !professionals.isLoading && professionalsBySelectedGroup && professionalsBySelectedGroup.length === 0 && <div
          ...
          {
              !professionals.isLoading && professionalsBySelectedGroup && paginetedProfessionals && paginetedProfessionals.currentTableData?.length > 0 && paginetedProfessionals.currentTableData.map((professional) => {
              return <CustomizedGroupAccordion
                defaultExpanded={false}
                expanded={currentProfessional?.id === professional.id}
                onChange={
                  () => {
                    if (currentProfessional?.id === professional.id) {
          ...
          <Pagination
            currentPage={paginetedProfessionals.currentPage}
            onChange={(page) => paginetedProfessionals.setCurrentPage(page)}
            totalPages={paginetedProfessionals.totalPage}
          />
