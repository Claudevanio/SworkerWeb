...
export function ModalFiltroServicosDashboard({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const onSubmit = (data: FormFields) => {
    ...
    const start = data.date
      ? dayjs(data.date, 'DD/MM/YYYY').startOf('day').toDate().toISOString()
      : period === 'today'
        ...
        : period === 'yesterday'
          ...
          : period === 'last7days'
            ...
            : period === 'last30days'
              ? dayjs().subtract(30, 'day').startOf('day').toDate().toISOString()
              : dayjs().subtract(7, 'day').startOf('day').toDate().toISOString();
  ...
  return (
    ...
    >
      <Form className="flex flex-col gap-4 md:gap-8" onSubmit={methods.handleSubmit(onSubmit)} {...methods}>
        ...
        />
        <Input label="Data" name="date" mask={masks.DATE} />
      </Form>
    </Modal>
  );
}
