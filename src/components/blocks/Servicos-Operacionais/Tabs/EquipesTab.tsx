...
function handleProfissionalData(tasksData: ServiceOrderTask[]): Array<ProfessionalData> {
  ...
  tasksData.forEach(task => {
    task.serviceOrdersOnDemand.forEach(singleTask => {
      addOrUpdateProfessionalData(
        singleTask.professionalId,
        singleTask.professionalName,
        Math.abs(dayjs(singleTask.taskCheckInDate).diff(dayjs(singleTask.taskCheckOutDate), 'second')),
        true
      );
    });
...
export function EquipesTab({ tasks, tasksIsLoading }: { tasks: ApiResponse; tasksIsLoading: boolean }) {
  ...
  const { currentTableData, currentPage, setCurrentPage } = usePagination<ProfessionalData>(professionalData, 5);
  ...
  const realizingTime = days.map(day => {
    return {
      date: day,
      value: demandData
        .concat(programmedData)
        .filter((task: singleTask) => dayjs(task.taskCheckInDate).format('DD/MM') === day)
        .reduce((acc, curr) => acc + Math.abs(dayjs(curr.taskCheckInDate).diff(dayjs(curr.taskCheckOutDate), 'second')), 0),
  ...
  const avgRealizingTime = days.map(day => {
    return {
      date: day,
      value:
        (demandData.find((task: singleTask) => dayjs(task.taskCheckInDate).format('DD/MM') === day)?.estimatedTime ?? 0) +
        (programmedData.find((task: singleTask) => dayjs(task.taskCheckInDate).format('DD/MM') === day)?.estimatedTime ?? 0),
      type: 'Tempo Médio'
    };
  });
  ...
  return (
    <div>
      <h2 className="text-base-7 text-lg md:text-2xl font-bold mb-4">Evolução de equipes</h2>
      <BaseTable
      ...
      />
      <Pagination currentPage={currentPage} onChange={page => setCurrentPage(page)} totalPages={Math.ceil(professionalData.length / 4)} />
      <div>
