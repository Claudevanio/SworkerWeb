...
export function PermissionsTab() {
        ...
        return (
          <div className="flex gap-2 flex-wrap">
            {permissionsArr.map((permission, index) => (
              <div key={index} className="bg-primary-100 text-primary-500 rounded-lg p-2">
                {permission.type} :{' '}
                {permissions.permissions?.find(p => p.value === permission.value)?.name ??
                  (permission.value === EtipoPermissao.Nada ? 'Nada' : 'Não encontrado')}
              </div>
            ))}
          </div>
        );
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      />
      <Pagination
        currentPage={permissions?.filters?.page ?? 1}
        totalPages={Math.ceil(permissions?.data?.count / permissions?.filters?.pageSize)}
        onChange={page =>
          permissions.setFilter(prev => ({
            ...prev,
            page
