...
export function ModalPermissions({ isOpen, onClose, current }: { isOpen: boolean; onClose: () => void; current?: IRole }) {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      methods={methods}
      title={!current ? 'Novo papel/permissão' : 'Editar papel/permissão'}
      onSubmit={() => methods.handleSubmit(onSubmit)()}
    >
      <Form onSubmit={data => onSubmit(data as FormFields)} className="flex flex-col gap-6" {...methods}>
        <Input name="name" label="Nome" required placeholder="Nome" />

        <div className="flex gap-4 flex-col">
          <h1 className="text-base-8 text-base font-semibold">
            Permissões<span className="text-erro-3">*</span>
          </h1>
          <div className="flex gap-2 flex-col">
            {fields.map(field => (
              <Dropdown key={field.name} name={field.name} label={field.label} options={field.options} required />
            ))}
          </div>
        </div>
      </Form>
