...
export function GestaoComponent() {
  ...
  const { modal } = useGestao();

  const handleLabel = (index: number) => {
    return subTab === 1 ? 'Adicionar Unidade' : subTab === 0 ? 'Adicionar Setor' : subTab === 2 ? 'Adicionar Profissional' : 'Adicionar Equipamento';
  };
  ...
  return (
    <>
      <div className="w-full p-4 lg:p-8">
        ...
        />
        <EmpresasTab activeTab={subTab} setActiveTab={setSubTab} />
      </div>
