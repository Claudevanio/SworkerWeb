            ...
            >
              <CheckBox
                ...
                onChange={() => {
                  ...
                  } else setSelected(prev => [...prev, row]);
                }}
              />
            </div>
            <span className="">{row.uid}</span>
          </div>
        );
      }
    },
    ...
    <>
      <div className="flex flex-col md:flex-row justify-between items-center w-full gap-4">
        ...
        <div className="flex justify-end md:justify-between items-center w-full">
          ...
          <ExportButton
            csvData={selected.length > 0 ? mapCSVData(selected) : mapCSVData([])}
      ...
      <Pagination
        ...
        totalPages={Math.ceil(equipments?.data?.count / equipments?.filters?.pageSize)}
        onChange={page =>
          equipments.setFilter(prev => ({
            ...prev,
            page
      ...
      />
      <ModalFiltroEquipament onClose={closeFilterModal} isOpen={isFilterModalOpen} />
      <ModalEquipmentsHistory onClose={closeHistoryModal} isOpen={isHistoryModalOpen} />
    </>
  );
}
