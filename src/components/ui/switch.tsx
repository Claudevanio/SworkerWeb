...
export function CustomSwitch({ label, ...props }: CheckboxProps & { label: string; control?: any; row?: boolean }) {
  return (
    <div
      ...
      style={props.row ? { flexDirection: 'row-reverse', alignItems: 'center', width: 'fit-content' } : {}}
    >
      <label className={props.className + ' text-primary font-semibold'}>{label}</label>
      <Controller
        name={props.name}
        control={props?.control}
        render={({ field }) => (
          <IOSSwitch {...field} checked={field.value} disabled={props.disabled} onChange={e => field.onChange(e.target.checked)} />
        )}
      />
    </div>
  );
